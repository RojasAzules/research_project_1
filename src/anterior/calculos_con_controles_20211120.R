...
if (precarga) {
  ...
  coeficiente <- casos %>%
    ...
    lm(formula = tasa_confirmados ~ interaccion + average_wage + average_age + 
         average_schooling + women_share + factor(semana) + factor(ut)) %>%
    coeftest(., vcov. = vcov(., type = "HC0"))
  estimacion <- rbind(estimacion, coeficiente[c('interaccion', 
                                                'average_wage',
                                                'average_age',
                                                'average_schooling',
                                                'women_share'), ])
  ...
  estimacion_cc <- estimacion
  leads_n_lags_cc <- leads_n_lags

  save(estimacion_cc, leads_n_lags_cc, file= 'data/interim/efecto_acumulado_cc.RData')
}
