...
## Latvia also speaks Belarrusian

Se repite la estrategia empírica pero esta vez observando el efecto relacionado a la cantidad de hablantes de bielorruso. Coloquemos en una tabla las semanas con efectos más significativos. Las semanas se indican como \verb+aaaass+, donde \verb+ss+ es el número de semana del año \verb+aaaa+.

```{r}
belarussian_por_semana %>% filter(semana != 202053) %>% arrange(`Pr(>|t|)`) %>% head(7) %>% 
  kbl(booktabs = T)  %>% kable_styling(position = "center",
                                       latex_options = "striped")
```

Podemos observar que nos encontramos ante un patrón distinto a los hablantes de ruso en cuanto a la aparición de efectos significativos con respecto a la proporción de hablantes de lengua bielorrusa en una unidad territorial dada.
...
belarussian_por_semana %>% 
  ...
  xlab('Fecha (2020-2021)') +
  labs(title = 'Nuevos casos por cada 100 mil habitantes',
       subtitle = 'Con respecto a la proporción de población bielorrusa')
...
- Realizar regresión de lead and lags con el tratamiento interactuado con la variable $rsp_m$. 

- Depurar la data de salarios mensuales promedio por unidad territorial.
